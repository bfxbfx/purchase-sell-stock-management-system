...
public class PlanBuyDetail extends JFrame {// 销售单内部窗体
	...
	public PlanBuyDetail() {// 销售单内部窗体的构造方法
...
//		setClosable(true);// 可以关闭销售单内部窗体
		...
		getContentPane().setBackground(new Color(187,219,194));
		setTitle("采购计划单据详情");// 设置销售单内部窗体的标题
		...
		setupComponet(new JLabel("采购计划单号："), 0,0,1,0,false);
		...
		// “销售时间”标签和“销售时间”文本框
		setupComponet(new JLabel("               生成时间："), 2, 1, 1, 0, false);
		...
		setupComponet(new JLabel(Global.xs_czy), 1, 4, 1, 1, true);

		
		// “出库”按钮
		final JButton tjButton = new JButton("关联生成采购单");
		
		tjButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				JFrame inFrm = new PlantoBuyBill(); 
...
private List searchInfo(List list) {
	String sql = "select * from tb_planbuy_detail where ";
