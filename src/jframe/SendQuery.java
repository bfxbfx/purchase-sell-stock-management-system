...
public class SendQuery extends JInternalFrame{// 销售查询内部窗体
	...
	public SendQuery() {// 销售查询内部窗体的构造方法
...
//				date.setDate(1);
		...
		condition.setModel(new DefaultComboBoxModel(new String[] {"客户全称", "发货单号"}));
		...
		showAllButton.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {
				content.setText("");
				List list=Dao.findForList("select * from tb_send");
		...
		String[] tableHeads = new String[]{"   发货单号", "   客户全称", "      发货日期","    金额","    经手人", "    结算方式","   操作员"};
		      ...
		      public void mouseClicked(MouseEvent e) { 
		    	  if(e.getClickCount() == 2) {
		    		   int row = ((JTable)e.getSource()).rowAtPoint(e.getPoint()); //获得行位置 
		    	  		...
		    	  		JFrame inFrm = new SendQueryDetail(); 
	...
	private final class QueryAction implements ActionListener {
		public void actionPerformed(final ActionEvent e) {
			...
			list = Dao.findForList("select * from tb_send where "
					+ con
					+ opstr
					+ "'"+cont+"'"
