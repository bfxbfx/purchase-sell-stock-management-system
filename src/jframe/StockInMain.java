...
public class StockInMain extends JInternalFrame{// 销售查询内部窗体
	...
	public StockInMain() {// 销售查询内部窗体的构造方法
...
//				date.setDate(1);
		...
		setTitle("采购到货信息查询");// 设置销售查询内部窗体的标题
		...
		condition.setModel(new DefaultComboBoxModel(new String[] {"供应商全称", "采购单号"}));
		...
		showAllButton.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {
				content.setText("");
				List list=Dao.findForList("select * from tb_buy where state = '待入库'");
		...
		showAllButton.setText("显示全部待入库采购单");
		// 滚动面板
		...
		String[] tableHeads = new String[]{"   采购单号", "   供应商全称", "      采购日期","    金额","    经手人", "    结算方式","   操作员",};
		      ...
		      public void mouseClicked(MouseEvent e) { 
		    	  if(e.getClickCount() == 2) {
		    		   int row = ((JTable)e.getSource()).rowAtPoint(e.getPoint()); //获得行位置 
		    	  		...
		    	  		JFrame inFrm = new StockInDetail(); 
	...
	private final class QueryAction implements ActionListener {
		public void actionPerformed(final ActionEvent e) {
			...
			list = Dao.findForList("select * from tb_buy where "
					+ con
					+ opstr
					+("state = '待入库'")
					+ "'"+cont+"'"
