...
public class InQuery extends JInternalFrame{// 销售查询内部窗体
...
//				date.setDate(1);
		...
		condition.setModel(new DefaultComboBoxModel(new String[] {"供应商全称", "入库单号"}));
		...
		showAllButton.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {
				content.setText("");
				List list=Dao.findForList("select * from tb_in");
		...
		String[] tableHeads = new String[]{"   入库单号", "   供应商全称", "      出库日期","    金额","    经手人", "    结算方式","   操作员"};
		      ...
		      public void mouseClicked(MouseEvent e) { 
		    	  if(e.getClickCount() == 2) {
		    		   int row = ((JTable)e.getSource()).rowAtPoint(e.getPoint()); //获得行位置 
		    	  		...
		    	  		JFrame inFrm = new InQueryDetail(); 
	...
	private final class QueryAction implements ActionListener {
		public void actionPerformed(final ActionEvent e) {
			...
			list = Dao.findForList("select * from tb_in where "
					+ con
					+ opstr
					+ "'"+cont+"'"
