...
public class SendQueryDetail extends JFrame {// 销售单内部窗体
	...
	public SendQueryDetail() {// 销售单内部窗体的构造方法
...
//		setClosable(true);// 可以关闭销售单内部窗体
		...
		setupComponet(new JLabel("发货票号："), 0,0,1,0,false);
...
DefaultTableCellRenderer r = new DefaultTableCellRenderer();   
	...
	private void updateTable(List list, final DefaultTableModel dftm) {
		...
		while (iterator.hasNext()) {
			...
			ypInfo = Dao.getFHInfo(item,Global.xs_number);
...
private List searchInfo(List list) {
	String sql = "select * from tb_send_detail where ";
