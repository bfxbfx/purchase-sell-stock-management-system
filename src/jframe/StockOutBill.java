...
public class StockOutBill extends JFrame {// 销售单内部窗体
	...
	public StockOutBill() {// 销售单内部窗体的构造方法
...
//		setClosable(true);// 可以关闭销售单内部窗体
		...
		getContentPane().setBackground(new Color(178,223,238));
		setTitle("出库单");// 设置销售单内部窗体的标题
		...
		setupComponet(new JLabel("出库单号："), 0,0,1,0,false);
		...
		// “出库”按钮
		final JButton tjButton = new JButton("   出库   ");
		...
		tjButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				TbSale stocklack = new TbSale(planid, jeStr,  kehuName, punchTime, czyStr, jsrStr, jsfsStr);// 采购计划主表
				...
				Set<TbSaleDetail> setl = stocklack.getTbSaleDetails();// 获得采购计划明细的集合
				...
				for (int i = 0; i < rows; i++) {
					...
					TbSaleDetail detail1 = new TbSaleDetail();// 销售明细
					detail1.setYpid(ypid);// 流水号
					detail1.setTbSellMain(stocklack.getSellId());// 销售主表
					detail1.setDj(dj);// 销售单价
					detail1.setSl(sl-rest);// 销售数量
					detail1.setName(ypname);
					detail1.setPlace(ypplace);
					detail1.setUnit(ypunit);
					detail1.setSpec(ypspec);
					set.add(detail);// 把销售明细添加到出库明细的集合中
					if(sl>rest) {
						flag=1;
						setl.add(detail1);
					}
				...
				if(flag==0) {
					boolean rs = Dao.insertOutInfo(sellMain);// 添加出库信息
					if (rs) {
						JOptionPane.showMessageDialog(StockOutBill.this, "出库完成");
						dispose();
					 ...
					 String[] options = {"提交采购计划单申请补货", "取消出库"};
				        int x = JOptionPane.showOptionDialog(null, "仓库缺货，出库失败！",
				                "操作失败",
				                JOptionPane.DEFAULT_OPTION, JOptionPane.INFORMATION_MESSAGE, null, options, options[0]);
				        if(x==0) {
				        	boolean rs = Dao.insertPlanBuyInfo(stocklack);// 添加采购计划信息
							if (rs) {
								JOptionPane.showMessageDialog(StockOutBill.this, "提交完成");
								dispose();
								hjje.setText("0");
							}
				        }
...
DefaultTableCellRenderer r = new DefaultTableCellRenderer();   
	...
	private void initPiaoHao() {
		java.sql.Date date = new java.sql.Date(System.currentTimeMillis());
		String maxId = Dao.getOutMainMaxId(date);
