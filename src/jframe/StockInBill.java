...
public class StockInBill extends JFrame {// 销售单内部窗体
	...
	public StockInBill() {// 销售单内部窗体的构造方法
...
//		setClosable(true);// 可以关闭销售单内部窗体
		...
		// “出库”按钮
		final JButton tjButton = new JButton("   入库   ");
		...
		tjButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < rows; i++) {
					...
					boolean rs = Dao.insertInInfo(sellMain);// 添加出库信息
					if (rs) {
						JOptionPane.showMessageDialog(StockInBill.this, "入库完成");
						dispose();
