...
public class PlanBuyMain extends JInternalFrame{// 销售查询内部窗体
	...
	public PlanBuyMain() {// 销售查询内部窗体的构造方法
...
//				date.setDate(1);
		...
		setTitle("采购计划单查询");// 设置销售查询内部窗体的标题
		...
		condition.setModel(new DefaultComboBoxModel(new String[] {"供应商全称", "采购计划单号"}));
		...
		showAllButton.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {
				content.setText("");
				List list=Dao.findForList("select * from tb_planbuy where state = '待处理'");
		...
		showAllButton.setText("显示全部未处理采购计划单");
		// 滚动面板
		...
		String[] tableHeads = new String[]{"   采购计划单号", "      申请日期","    金额","    经手人", "    结算方式","   操作员",};
		      ...
		      public void mouseClicked(MouseEvent e) { 
		    	  if(e.getClickCount() == 2) {
		    		   int row = ((JTable)e.getSource()).rowAtPoint(e.getPoint()); //获得行位置 
		    	  		...
		    	  		Global.xs_date=(String) table.getValueAt(row, 1);
		    	  		Global.xs_jr=(String) table.getValueAt(row, 2);
		    	  		Global.xs_jsr=(String) table.getValueAt(row, 3);
		    	  		Global.xs_jsfs=(String) table.getValueAt(row, 4);
		    	  		Global.xs_czy=(String) table.getValueAt(row, 5);
		    	  		
		    	  		JFrame inFrm = new PlanBuyDetail(); 
	...
	private final class QueryAction implements ActionListener {
		public void actionPerformed(final ActionEvent e) {
			...
			list = Dao.findForList("select * from tb_planbuy where "
					+ con
					+ opstr
					+("state = '待处理'")
					+ "'"+cont+"'"
