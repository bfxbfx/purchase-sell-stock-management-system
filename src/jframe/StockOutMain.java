...
public class StockOutMain extends JInternalFrame{// 销售查询内部窗体
	...
	public StockOutMain() {// 销售查询内部窗体的构造方法
...
//				date.setDate(1);
		...
		setTitle("已销售信息查询");// 设置销售查询内部窗体的标题
		...
		showAllButton.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {
				content.setText("");
				List list=Dao.findForList("select * from tb_sale where (state = '待出库' or state = '仓库缺货，待补货') ");
		...
		showAllButton.setText("显示全部未处理销售单");
		// 滚动面板
		...
		String[] tableHeads = new String[]{"   销售票号", "   客户全称", "      销售日期","    金额","    经手人", "    结算方式","   操作员",};
		      ...
		      public void mouseClicked(MouseEvent e) { 
		    	  if(e.getClickCount() == 2) {
		    		   int row = ((JTable)e.getSource()).rowAtPoint(e.getPoint()); //获得行位置 
		    	  		...
		    	  		JFrame inFrm = new StockOutDetail(); 
	...
	private final class QueryAction implements ActionListener {
		public void actionPerformed(final ActionEvent e) {
			...
			list = Dao.findForList("select * from tb_sale where "
					...
					+ opstr
					+ "'"+cont+"'"
					+("and (state = '待出库' or state = '仓库缺货，待补货') ")
